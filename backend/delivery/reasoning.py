...
async def describe_ambient(
    ...
    *,
    ...
    instruction = (
        ...
        "smaller.\n"
        "- `what_is_standing_right_now` is what ORA has already decided is "
        "worth this person's attention, and it is on the same screen as your "
        "line, just below it. If there is anything there, «tutto tranquillo», "
        "«nessun problema», «niente da segnalare» are false, whatever the "
        "review found: say what was looked at and that one of them wants "
        "their attention. Do not repeat what it says — it says it itself, "
        "right underneath.\n"
        "- no reassurance theatre: never «sto lavorando per te», «sto "
        "monitorando», «ho controllato tutto».\n"
        "- no implementation words. Nobody says «scan», «opportunità», "
        "«segnalazione», «fonti» or «controllo» about their own week. You "
        "looked in their calendar, in their mail, at what had been noted: "
        "say it the way they would.\n"
        "- if `what_is_standing_right_now` is empty, there is nothing on "
        "their screen for your line to be about. Do not announce things they "
        "cannot see: say what was looked at, and no more. A line that "
        "promises something the screen does not hold is worse than no "
        "line.\n"
