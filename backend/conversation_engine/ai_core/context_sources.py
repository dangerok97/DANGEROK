...
class ContextSourceRegistry:
    ...
    def select(
        self, need: ContextNeed, *, maximum: int
    ) -> tuple[List[ContextSource], Dict[str, str]]:
        hints = {str(x).strip().lower() for x in need.source_hints if str(x).strip()}
        query_tokens = _tokens(
            " ".join([need.query, need.purpose or "", *need.desired_evidence])
        )
        scored = []
        ...
        for source in self.list():
            ...
            if source.policy != "automatic":
                ...
                continue
            descriptor = _tokens(
                f"{source.name} {source.description} {source.temporal_nature}"
            )
            overlap = len(query_tokens & descriptor)
            hint = 5 if source.name in hints else 0
            # Situation, Profile and governed Memory are generic personal-context
            # anchors once the AI explicitly requests Stage B. This prevents a
            # language-dependent zero-overlap tie from excluding durable evidence.
            anchor = 2 if source.name in ("situations", "profile", "memory") else 0
    ...
    def _register_defaults(self) -> None:
        specs = (
            (
                "profile",
                "confirmed and user-stated personal profile facts",
                "user-confirmed",
                "personal",
                "durable",
                self._profile,
            ...
            (
                "memory",
                "durable personal memory statements with epistemic authority",
                "mixed",
                "personal",
                "durable",
                self._memory,
            ...
            (
                "situations",
                "active or recently changed contextual situations",
                "user-said",
                "personal",
                "temporal",
                self._situations,
            ...
            (
                "life_os",
                "active plans, next actions, progress and generated objects",
                "system-structured",
                "personal",
                "operational",
                self._life_os,
            ...
            (
                "goals",
                "active outcomes, deadlines, state and next actions",
                "system-structured",
                "personal",
                "operational",
                self._goals,
            ...
            (
                "files",
                "user file metadata and untrusted document evidence references",
                "document-backed",
                "sensitive",
                "mixed",
                self._files,
            ...
            (
                "calendar",
                "read-only user calendar draft metadata already available locally",
                "document-backed",
                "sensitive",
                "temporal",
                self._calendar,
        ...
        for name, desc, authority, sensitivity, temporal, fn in specs:
            self.register(
                ContextSource(name, desc, authority, sensitivity, temporal, fn)
            )
        self.register(
            ContextSource(
                "presence",
                "live consent-aware device presence",
                "device-signal",
                "highly_sensitive",
                "ephemeral",
    ...
    async def _memory(
        ...
        response = await get_life_memory_service(self.db).get_life_memory(
        ...
        for memory in list(getattr(response, "memories", None) or [])[:30]:
            ...
            if statement:
                display, ref, status, provenance = _memory_fact_payload(memory)
                out.append(
                    _fact(
                        display,
                        "memory",
        ...
        for plan in await repo.list_active_plans(user_id, limit=12):
            ...
            out.append(
                _fact(
                    ...
                    plan.updated_at,
                    f"plan:{plan.id}",
                    provenance=list(str(x) for x in plan.evidence_refs[:4]),
                )
            )
        return out
    ...
    async def _goals(
        ...
        return [
            _fact(
                ...
                f"goal:{g.id}",
                provenance=[
                    str((g.created_from or {}).get("source_type") or "goal_engine")
                ],
            )
            for g in goals
        ]
    ...
    async def _files(
        ...
        if session_id:
            ...
            for item in await ContextFileService(self.db).list_session_files(
                user_id, session_id
            ):
                name = str(
                    item.get("name") or item.get("original_name") or "user file"
                )[:120]
    ...
    async def _presence_unavailable(
        self, user_id: str, need: ContextNeed, session_id: Optional[str]
    ) -> List[ContextFact]:
        return []
...
def rank_evidence(
    ...
    *,
    ...
    query_tokens = _tokens(
        " ".join([need.query, need.purpose or "", need.temporal_scope or "", anchor])
    )
    source_hints = {
        str(value).strip().lower() for value in need.source_hints if str(value).strip()
    }
    authority = {
        "user-confirmed": 5,
        "document-backed": 4,
    ...
    for index, fact in enumerate(candidates):
        ...
        # A validated AI hint expresses the evidence class needed for this step.
        # It must outrank incidental lexical overlap from the Situation anchor,
        # while registry policy and final budgets still bound what can be read.
        hinted_source = 50 if fact.source.lower() in source_hints else 0
        score = (
            overlap * 4 + authority.get(fact.authority, 2) + exact_ref + hinted_source
        )
...
def _fact(
    ...
    *,
    ...
    return ContextFact(
        ...
        provenance=(provenance or [])[:6],
        sensitivity=sensitivity,
    )


def _memory_fact_payload(memory: Any) -> tuple[str, str, str, List[str]]:
    statement = str(getattr(memory, "statement", ""))
    governed_ref = next(
        (
            str(ref).split("user_memory:", 1)[1]
            for ref in (getattr(memory, "source_refs", None) or [])
    ...
    if governed_ref:
        return (
            "Governed durable Memory (the supplied ref is an actionable mutation target): "
            + statement,
            governed_ref,
            "active_governed",
            ["life_memory", "governed_memory"],
        )
    readonly_id = str(getattr(memory, "id", None) or "memory")
    return (
        "Read-only derived personal context (not a governed Memory mutation target): "
        + statement,
        f"memory_read_only:{readonly_id}",
        "read_only_evidence",
        ["life_memory"],
    )
...
def _unwrap_profile(obj: Any) -> tuple[Any, str, str, bool, Optional[str]]:
    if hasattr(obj, "value"):
        return (
            ...
            str(getattr(obj, "source", "user_said")),
            str(getattr(obj, "status", "extracted")),
