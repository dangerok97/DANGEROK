"""Groq chat adapter — OpenAI-compatible API (no extra package required)."""
...
logger = logging.getLogger("ora.llm.groq")
...
class GroqProvider(BaseLLMProvider):
    ...
    def model_name(self) -> Optional[str]:
        return (os.environ.get("GROQ_MODEL") or "qwen/qwen3.8-27b").strip()
    ...
    async def chat(
        ...
        *,
        ...
        if not api_key:
            raise LLMNotConfigured("GROQ_API_KEY mancante")
        model = self.model_name() or "qwen/qwen3.8-27b"
        timeout_s = float(os.environ.get("GROQ_TIMEOUT_S") or os.environ.get("LLM_TIMEOUT_S") or "60")
    ...
    async def _chat_openai_sdk(
        ...
        *,
        ...
        client = AsyncOpenAI(api_key=api_key, base_url=_GROQ_BASE, timeout=timeout_s)
        kwargs: Dict[str, Any] = {
            "model": model,
            "messages": [
