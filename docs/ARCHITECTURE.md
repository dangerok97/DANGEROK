...
Quando il modello restituisce una struttura non valida o almeno una proposta non valutabile, OpportunityService.scan segnala unavailable. OpportunityDiscovery rilascia il batch di cambiamenti, registra solo l'ora del tentativo e applica il cooldown di 120 secondi; non salva il fingerprint, così gli stessi fatti possono essere rivalutati. Le opportunità valide in un batch parziale restano persistite e l'identità evita di crearne copie al retry. Una lista di opportunità esplicitamente vuota resta silenzio intenzionale e consuma il batch. La modifica è su ramo di revisione, non distribuita.
