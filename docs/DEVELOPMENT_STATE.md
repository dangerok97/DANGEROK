...
Una prova successiva su Staging ha incontrato risposte JSON del modello oltre i limiti di alcuni campi annidati di `ResearchPlan` e `ResearchAssessment`. La correzione riduce solo la lunghezza di quei campi prima della validazione e registra il tipo e la posizione degli errori rimanenti, senza contenuto personale. La ricerca continua a ritentare dopo dodici ore quando non è completata; resta da riprovare il percorso live dopo questa correzione.

Bollette e polizze riconosciute creano un monitor persistente che lavora ogni sette giorni nel runtime ambientale. La ricerca web live e il monitor sono attivi su ORA Staging: prove sintetiche hanno trovato pagine ufficiali per luce e RC auto senza app aperta. Ogni controllo impone una nuova ricerca, seleziona solo pagine del venditore citate e passa le alternative al giudizio delle opportunità. Il nuovo consiglio di risparmio usa un confronto limitato alla componente di vendita: prezzo unitario e quota commerciale fissa devono essere espliciti nella bolletta e nello snippet della fonte citata, con consumo annuo non stimato e senza OCR non confermato. Non equivale a un preventivo totale né prova eleggibilità; per polizze e dati incompleti ORA indica il confronto concreto da fare senza cifra. I risultati più vecchi di dieci giorni spariscono; un errore riprova dopo dodici ore. Prima di considerare completo il consiglio quantificato: CI, test sintetico end to end su Staging e verifica della resa UI. Un ciclo settimanale intero non è ancora stato osservato.
...
L'autorizzazione non comprende automaticamente azioni verso persone,
pagamenti, operazioni distruttive o modifica delle credenziali.
