...
2. V3.21.4: mount persistente, sessioni/health, sync cloud, segnali posizione e readiness telefonia non sono più blocker. Restano due reality gate — modifica di un evento Google esistente **da ORA**; negazione/revoca e riattivazione posizione con meteo sul device — più un blocker sicurezza: vecchi blob `backend/data/documents` sono ancora raggiungibili nella storia del repository pubblico e richiedono bonifica esplicita della history.
