...
| Bolletta luce | ✅ | ✅ | ✅ | ✅ Gemini reale (conf. 1.00) | ✅ | **pytest + pytest+Gemini reale + Playwright (scenario BOLLETTA)** |
...
| Suite | Risultato |
|---|---|
| `backend/ai_life_strategist/tests/test_life_experience_documents.py` (nuova, questa feature) | **62 passed** |
| `backend/life_setup` + `backend/ai_life_strategist` + `backend/documents` (regressione completa, pacchetti toccati) | **92 passed** |
| `python -m compileall life_setup documents ai_life_strategist tests` | OK, nessun errore di sintassi |

## Playwright — risultati reali

| Spec | Scenario | Esito |
|---|---|---|
| `frontend/e2e/life-experience-documents.spec.ts` | CASA: rogito → capito → correggi campo → conferma → re-plan → Home benefit → persistenza (reload + re-login) | ✅ passed |
| `frontend/e2e/life-experience-documents.spec.ts` | AUTO: libretto → capito (targa `AB123CD` riconosciuta) → conferma → Home benefit auto | ✅ passed |
| `frontend/e2e/life-experience-documents.spec.ts` | BOLLETTA: bolletta luce → fornitore/importo riconosciuti → proposta evento (draft, mai auto-creato) → Home/Proactive | ✅ passed |
| `frontend/e2e/life-experience-ai.spec.ts` (esistente, regressione) | Conversazione + upload rogito + explain + exit; conversazione→rogito→piano→interrupt→resume→Home→Proactive | ✅ 2/2 passed |
...
- Google Calendar: gli eventi da scadenza documento restano **draft-only**; la conferma verso Google reale non è stata rieseguita in questa sessione (il gate di consenso esiste dal lavoro Documents V2 precedente, riusato qui senza modifiche).
